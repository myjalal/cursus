...
void	sort_short(t_stk **a, t_stk **b, int len, t_var *v)
{
	...
	tmp = NULL;
	if (len == 2 && (*a)->num > (*a)->nxt->num)// dont need (*a)->num > (*a)->nxt->num because already check if it's sorted
		pick_case(a, b, SA, v);
	if (len <= 3)
