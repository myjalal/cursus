...
while [ $i -lt $NUM_RUNS ]
	do
	ARG=`ruby -e "puts (-200..200).to_a.shuffle.join(' ')"`; ./push_swap $ARG | ./checker_Mac $ARG >> './celinette_checker'
	i=$[$i+1]
done
printf "${BLUE}\n*** number of KO = \t ${NC}"
...
while [ $i -lt $NUM_RUNS ]
	do
	ARG=`ruby -e "puts (-100..100).to_a.shuffle.join(' ')"`; ./push_swap $ARG | ./checker_Mac $ARG >> './celinette_checker'
	i=$[$i+1]
done
printf "${BLUE}\n*** number of KO = \t ${NC}"
